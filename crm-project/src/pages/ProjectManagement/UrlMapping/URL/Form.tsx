...
const UrlMappingForm = () => {
  ...
  const { compliance, assessment_types } = useSelector(
    (state: RootState) => state.assessment
  );
  const { clientdetail, client_assesment_id } = useSelector(
    (state: RootState) => state.project
  );
  const { employees } = useSelector((state: RootState) => state.myTeam);

  const { data: clientAssessment } = useGetClientAssessmentTypeQuery(
    {
  ...
  const assessmentOptions = useMemo(() => {
    return (
      clientAssessment?.results.map((item: any) => ({
  ...
  const pre_select_assessment_option = useMemo(() => {
    if (client_assesment_id?.assessment_type && clientAssessment?.results) {
      const match = clientAssessment.results.find(
        (item: any) => item.id === client_assesment_id.assessment_type
      );
      if (match) return match.assessment_type_name;
    }
    return searchParams.get("assessment_type");
  }, [client_assesment_id, clientAssessment, searchParams]);
  ...
  useEffect(() => {
    if (client_assesment_id) {
      setFormData((prev: any) => ({
        ...prev,
        client_assessment_id: Number(client_assesment_id?.id),
      }));
    }
  }, [client_assesment_id, setFormData]);
  ...
  return (
    <ComponentCard title="Add URL Mapping">
      ...
      <Form onSubmit={handleSubmit} className="space-y-4 mt-4 p-5">
          <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              ...
              <InputSelect
                ...
                defaultValue={pre_select_assessment_option} 
                onSelect={(opt) =>
                  handleSelectChange("client_assessment_id", opt)
                }
